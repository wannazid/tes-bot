...
class HostResponse:
    ...
    async def Result(self):
        ...
        async def process_domain(data):
            ...
            return result
        tasks = []
        if subdo is not None:
            tasks = [process_domain(data) for data in subdo]
        elif reverseip is not None:
            tasks = [process_domain(data) for data in reverseip]
        if tasks:
...
async def scan_host(message):
    ...
    if is_valid_ip(target):
        await bot.reply_to(message, "Pemindaian IP sedang berlangsung, harap tunggu!")
        host_response = HostResponse(target, user_agent, proxy)
        text_page = await host_response.Reverseip()
    else:
        await bot.reply_to(message, "Pemindaian domain sedang berlangsung, harap tunggu!")
        host_response = HostResponse(target, user_agent, proxy)
        text_page = await host_response.Subdomain()
